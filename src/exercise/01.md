...
- React.Suspence is nice for async data loads
- We can use a React.Suspense component around any component that requires data
  in order to be rendered. The React.Suspense takes a fallback UI that will be
  rendered until data is received.
- To render the fallback component at first, our component should throw an
  un-resolved promise (see example). Once the promise is resolved, React will
  call a re-render on our component. This time since the promise is resolved,
  our component can be rendered
