...
- The `useTransition` hook can be used to improve loading states. This hook
  returns a method that starts a transition and a boolean for whether or not the
  component is pending
- Code that triggers a suspending component should be wrapped in
  `startTransition`
- If the object passed to `useTransaction` has `timeoutMs` set to `4000`, then
  React will wait 4 seconds before displaying the Suspence fallback component
- In the meantime, the `isPending` variable can be used to manipulate styles to
  show that data is loading
- The other properties set in the `SUSPENSE_CONFIG` object in this exercise are
  a bit experimental, but they basically make it so that if we are already
  pending for `300ms`, then pend for `1000ms`
