...
- Caching improves the user experience for when a user fetches data for one
  pokemon name, then eventually comes back to it. Instead of fetching the data
  again we can just fetch it from the cache
- It can be nice to use a cache within context and ref hooks so that the cache
  is available in context of the React lifecycle
- Once a cache is set up within the React lifecycle, useEffect can be used to
  remove items from the cache when they should be invalidated (likely after a
  certain amount of time)
- Note, cacheing is a HARD problem, and this pattern should not be used over an
  abstracted, complete solution
