...
- To clean up this code a bit more, the useTransition, useState, and useEffect
  calls can be refactored into a custom hook, which here we call
  `usePokemonResource`. Once this logic is in a custom hook, it can be used by
  any component, and that component won't need to call all these things itself
- Once this logic is refactored into a custom hook, that custom hook can be
  moved to its own file and then can be reused throughout the application
  (however something is broken when trying to use the one in `src/pokemon.js`)
